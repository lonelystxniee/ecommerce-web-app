...
export const SectionHeading = ({ title, outlined }) => (
  <div className="relative flex items-center justify-center my-10 z-1">
    ...
    >
      <img
        alt="left"
        src={outlined ? 'https://honglam.vn/_next/static/media/btn41-bg-left-hover.a799d898.png' : 'https://honglam.vn/_next/static/media/btn47-bg-left-hover-solid.5a0f365f.png'}
        className="absolute -top-px -left-3 h-[calc(100%+2px)] w-[14px] object-contain"
      />
      <img
        alt="right"
        src={outlined ? 'https://honglam.vn/_next/static/media/btn41-bg-right-hover.5de6cf95.png' : 'https://honglam.vn/_next/static/media/btn47-bg-right-hover-solid.81fa6bf3.png'}
      ...
      <div className="px-6 flex items-center justify-center min-w-50 md:min-w-77.5 h-10">
        <h3 className={`${outlined ? 'text-primary' : 'text-white'} text-xl md:text-2xl lg:text-[30px] capitalize font-seagull font-bold tracking-tighter leading-none flex items-center`}>{title}</h3>
      </div>
    </div>
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const [prodRes, catRes] = await Promise.all([fetch(`${API_URL}/api/products?limit=100`), fetch(`${API_URL}/api/category`)])

        const prodData = await prodRes.json()
        const catData = await catRes.json()
        const adsRes = await fetch(`${API_URL}/api/ads`)
        const adsData = await adsRes.json()
        const artRes = await fetch(`${API_URL}/api/articles?limit=3&status=Hiện&type=news`)
        const artData = await artRes.json()

        const vidRes = await fetch(`${API_URL}/api/articles?limit=3&status=Hiện&type=video`)
  ...
  const leftSidebarAds = ads.filter((ad) => ad.type === 'sidebar' && ad.status === 'ACTIVE' && ad.position === 1)
  const rightSidebarAds = ads.filter((ad) => ad.type === 'sidebar' && ad.status === 'ACTIVE' && ad.position === 2)
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-transparent text-primary">
        ...
        <button onClick={() => window.location.reload()} className="px-6 py-2 bg-primary text-white rounded-full hover:bg-[#800a0d] transition-colors">
          Thử lại
        </button>
      </div>
    )
  ...
  return (
    <div className="min-h-screen pb-20 bg-transparent">
      ...
      {/* HERO */}
      <Hero categories={categories} ads={ads} />
          ...
          <div className="grid grid-cols-2 border border-[#c0a884] md:grid-cols-3 md:gap-4 lg:grid-cols-4">
            ...
            <div className="border-[0.5px] border-[#ded3c2] md:border-0">
              <div className="flex flex-col items-center justify-center gap-2.5 p-5 md:flex-row">
                <div>
                  <img src="https://honglam.vn/_next/static/media/menu-cs-1.e3475999.png" alt="giao-hang-sieu-toc" className="w-full h-full" />
                </div>
                <div className="text-secondary-2">
                  <h3 className="text-sm text-center md:text-left md:text-base md:font-bold text-text-primary">Giao hàng siêu tốc</h3>
                  <p className="hidden text-[13px] md:block text-text-primary">Giao hàng trong 24h</p>
                </div>
              </div>
            </div>
            ...
            <div className="border-[0.5px] border-[#ded3c2] md:border-0">
              <div className="flex flex-col items-center justify-center gap-2.5 p-5 md:flex-row">
                <div>
                  <img src="https://honglam.vn/_next/static/media/menu-cs-2.98ae416c.png" alt="tu-van-mien-phi" className="w-full h-full" />
                </div>
                <div className="text-secondary-2">
                  <h3 className="text-sm text-center md:text-left md:text-base md:font-bold text-text-primary">Tư vấn miễn phí</h3>
                  <p className="hidden text-[13px] md:block text-text-primary">Đội ngũ tư vấn tận tình</p>
                </div>
              </div>
            </div>
            ...
            <div className="border-[0.5px] border-[#ded3c2] md:border-0">
              <div className="flex flex-col items-center justify-center gap-2.5 p-5 md:flex-row">
                <div>
                  <img src="https://honglam.vn/_next/static/media/menu-cs-3.43ac297a.png" alt="thanh-toan" className="w-full h-full" />
                </div>
                <div className="text-secondary-2">
                  <h3 className="text-sm text-center md:text-left md:text-base md:font-bold text-text-primary">Thanh toán</h3>
                  <p className="hidden text-[13px] md:block text-text-primary">Thanh toán khi nhận hàng</p>
                </div>
              </div>
            </div>
            ...
            <div className="border-[0.5px] border-[#ded3c2] md:border-0">
              <div className="flex flex-col items-center justify-center gap-2.5 p-5 md:flex-row">
                <div>
                  <img src="https://honglam.vn/_next/static/media/menu-cs-4.ada696ef.png" alt="giai-phap-tieu-dung" className="w-full h-full" />
                </div>
                <div className="text-secondary-2">
                  <h3 className="text-sm text-center md:text-left md:text-base md:font-bold text-text-primary">Tiết kiệm tối ưu</h3>
                  <p className="hidden text-[13px] md:block text-text-primary">Ưu đãi cho đơn hàng lớn</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="px-4 mx-auto mt-10 text-center max-w-300 max-h-112.5">
        ...
        <div className="grid items-stretch grid-cols-2 grid-rows-3 gap-5 mt-8 md:grid-rows-2 md:grid-cols-3 max-h-112.5">
          ...
          <div className="relative h-full">{products[0] && <OrangeCard product={products[0]} />}</div>

          {/* GIỮA – CHIẾM 2 HÀNG (Sản phẩm index 1) */}
          <div className="relative h-full row-span-2 group">
            <div onClick={() => navigate(`/product/${products[1]?.id}`)} className="flex flex-col items-center w-full h-full p-8 overflow-hidden text-white shadow-md cursor-pointer ">
              <img src="https://honglam.vn/_next/static/media/bg-hot-product-3.1f862014.jpg" alt="" className="absolute inset-0 w-full h-full" />
              <h3 className="relative mb-1 md:text-[26px] font-bold tracking-tight uppercase z-1 font-seagull ">{products[1]?.name}</h3>
              <p className="relative mb-2 text-sm md:text-[15px] z-1">{products[1]?.slogan}</p>

              <img src={products[1]?.images?.[0] || products[1]?.image} className="object-cover w-24 h-24 mb-1 rounded-full md:w-32 md:h-32 lg:w-48 lg:h-48 drop-shadow-2xl" alt="" />

              <p className="mb-1 text-sm md:text-[15px] relatve z-1">Chỉ từ</p>
              <p className="mb-4 italic font-bold md:mb-6 md:text-3xl font-uvnvan relatve z-1">55.000đ</p>
          ...
          <div className="relative h-full">{products[2] && <OrangeCard product={products[2]} />}</div>

          {/* CỘT 1 - DƯỚI (Sản phẩm 3) */}
          <div className="relative h-full">{products[3] && <OrangeCard product={products[3]} />}</div>

          {/* CỘT 3 - DƯỚI (Sản phẩm 4) */}
          <div className="relative h-full">{products[4] && <OrangeCard product={products[4]} />}</div>
        </div>
      </div>

      {/* SECTION 8: TẤT CẢ SẢN PHẨM (PAGINATION) */}
      <div ref={allProductRef} className="px-4 mx-auto mt-40 max-w-300">
      ...
      <div className="px-4 mx-auto mt-20 text-center max-w-300">
        ...
        <div className="grid grid-cols-1 gap-8 mt-10 md:grid-cols-3">
          {products.slice(5, 8).map((p) => (
            <GiftCard key={p.id} id={p.id} title={p.name} price={Math.floor(p.price / 1000)} img={p.image} />
          ))}
        </div>
        <div className="flex justify-center mt-8">
          <Link to="/category/giai-phap-qua-tang" className="hover:text-primary group inline-flex items-center gap-2 text-base text-[#917359] font-bold transition-all underline-offset-4">
            Xem tất cả sản phẩm <img src="https://honglam.vn/_next/static/media/btn-more.c2bbf147.png" className="h-4 transition-all group-hover:translate-x-2" alt="more" />
          </Link>
        </div>
      </div>
      ...
      {categories.slice(0, 4).map((cat, index) => {
        ...
        const categoryProducts = allProducts.filter((p) => (p.categoryID || []).some((c) => c._id === cat._id || c.name === cat.name))
        ...
        <div className="mt-6 space-y-5">
          {articles.length > 0 ? (
            articles.map((article) => (
              <div key={article._id} className="flex gap-4 cursor-pointer group" onClick={() => navigate(`/magazine/${article._id}`)}>
                <div className="h-fit border-2 border-[#e0be91] p-1 shrink-0 bg-white">
                  <img
                    src={article.image || 'https://via.placeholder.com/300'}
                    alt="post"
                    className="h-25 w-37.5 max-w-none object-cover md:h-30 md:w-44.25 transition-transform duration-500 group-hover:scale-105"
                  />
                </div>
                <div className="flex flex-col justify-center">
                  <h3 className="text-base font-bold transition-all text-secondary-2 group-hover:text-primary line-clamp-2 md:text-lg">{article.title}</h3>
                  <div className="mt-2 text-sm text-secondary-2 line-clamp-3">{article.summary}</div>
                </div>
              </div>
            ))
          ) : (
            <p className="text-center text-[#88694f] italic">Chưa có bài viết nào...</p>
          )}
        </div>
      </div>
      ...
      <div className="px-4 mx-auto mt-20 max-w-300">
        ...
        <div className="mt-6">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            {/* Video lớn */}
            {videos.length > 0 ? (
              <a href={videos[0].link} target="_blank" rel="noopener noreferrer" className="relative flex flex-col h-full cursor-pointer group">
                <img src={videos[0].image || videos[0].thumbnail} alt={videos[0].title} className="flex-1 object-cover w-full h-80" />
                <div className="flex items-center gap-2 mt-4">
                  ...
                  <span className="text-xl font-bold text-primary line-clamp-2">{videos[0].title}</span>
                </div>
              </a>
            ) : (
              <div className="flex items-center justify-center italic h-60 bg-stone-100 text-stone-500">Chưa có video nổi bật</div>
            )}

            {/* Grid video nhỏ */}
            <div className="grid grid-cols-2 gap-3">
              {videos.slice(1).map((video, index) => (
                <a key={index} href={video.link} target="_blank" rel="noopener noreferrer" className="relative block cursor-pointer group">
                  <img src={video.image || video.thumbnail} alt={video.title} className="object-cover w-full h-40" />
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-sm font-medium line-clamp-2 text-primary">{video.title}</span>
                  </div>
                </a>
              ))}
            </div>
...
const ServiceItem = ({ img, text, last }) => (
  <div className={`flex items-center gap-3 justify-center ${!last ? 'border-r border-[#ded3c2]' : ''} px-4 py-2`}>
    <img src={img} className="h-8" alt="" />
    <div className="text-primary uppercase text-[11px] font-bold text-left leading-tight">{text}</div>
  </div>
)
...
const OrangeCard = ({ product }) => {
  ...
  return (
    <div className="bg-[#f39200] p-4 text-white flex justify-between shadow-sm h-full relative overflow-hidden">
      <img src="https://honglam.vn/_next/static/media/bg-hot-product-3.1f862014.jpg" alt="" className="absolute inset-0 w-full h-full brightness-110" />
      <div className="flex flex-col items-start flex-1 overflow-hidden text-left justify-evenly">
        <h3 className="relative text-xs font-bold line-clamp-1 sm:text-base lg:text-xl z-1">{product?.name}</h3>
        ...
        <p className="relative text-xl lg:text-[30px] z-1 font-uvnvan italic mb-2">{product?.price.toLocaleString('vi')}đ</p>
        <button
      ...
      <img
        ...
        className="self-center object-cover w-24 h-24 transition-transform rounded-full md:w-28 md:h-28 lg:h-44 lg:w-44 drop-shadow-md group-hover:rotate-6"
      />
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 z-20 flex items-center -translate-x-1/2 -translate-y-1/2 left-1/2">
        ...
        >
          <img alt="left" src="https://honglam.vn/_next/static/media/btn41-bg-left-hover-solid-2.6831ea9d.png" className="absolute -top-px -left-2.75 h-[calc(100%+2px)] w-3.5 object-contain" />
          <div className="flex items-center justify-center bg-secondary">
            <h3 className="flex items-center h-10 px-6 text-sm font-bold tracking-tighter capitalize md:text-base text-primary whitespace-nowrap">{title}</h3>
          </div>
          <img alt="right" src="https://honglam.vn/_next/static/media/btn41-bg-right-hover-solid-2.6d26b9cf.png" className="absolute -top-px -right-3 h-[calc(100%+2px)] w-3.5 object-contain" />
        </div>
      </div>

      {/* ẢNH SẢN PHẨM: Thêm Link để click vào ảnh là chuyển trang */}
      <Link to={`/product/${id}`} className="block overflow-hidden bg-white">
        <img src={img} className="object-cover w-full transition-transform duration-500 h-80" alt={title} />
      </Link>

      <div className="relative flex items-center justify-between px-4 py-5 overflow-hidden bg-transparent">
        <img src="https://honglam.vn/_next/static/media/bg_img.aee452a5.png" className="absolute left-0 object-cover w-full h-8 bottom-full z-3" alt="" />
...
const LuckyWheelButton = () => {
  return (
    <Link to="/lucky-wheel" className="fixed bottom-24 right-6 z-[100] flex flex-col items-center group">
      <div className="relative">
        ...
        <div className="relative bg-gradient-to-br from-[#9d0b0f] to-[#f39200] p-4 rounded-full shadow-2xl border-4 border-white transform group-hover:scale-110 transition-all duration-300 active:scale-95">
          <Trophy className="w-8 h-8 text-white md:w-10 md:h-10 animate-bounce" />
        </div>
        <div className="absolute right-full mr-4 top-1/2 -translate-y-1/2 bg-white px-4 py-2 rounded-xl shadow-xl border border-[#9d0b0f]/20 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-4 group-hover:translate-x-0 pointer-events-none">
          <p className="text-[#9d0b0f] font-black text-xs uppercase tracking-widest">Săn Voucher 🎁</p>
