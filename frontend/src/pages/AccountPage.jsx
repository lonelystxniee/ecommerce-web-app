...
const OrderList = ({ userId }) => {
  ...
  const handleCancelOrder = async (reason) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/api/orders/cancel/${orderToCancel._id}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (data.success) {
        ...
        const updatedRes = await fetch(
          `${API_URL}/api/orders/my-orders/${userId}`,
        );
  ...
  const handleRequestReturn = async (orderId) => {
    if (
      !window.confirm(
        "Bạn có chắc chắn muốn yêu cầu hoàn trả đơn hàng này? Shipper sẽ liên hệ để lấy lại hàng.",
      )
    )
      return;
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `${API_URL}/api/orders/request-return/${orderId}`,
        {
          method: "PUT",
          ...
          {[...Array(totalPages)].map((_, i) => (
            <button
              ...
              className={`w-9 h-9 rounded-full font-black text-xs transition-all ${
                currentPage === i + 1
                  ? "bg-[#800a0d] text-white shadow-lg scale-110"
  ...
  if (wishlistItems.length === 0) {
    return (
      <div className="p-12 text-center bg-white border border-gray-100 shadow-2xl rounded-[40px] animate-fadeIn">
        ...
        <p className="max-w-xs mx-auto mb-8 text-sm italic text-gray-400">
  ...
  useEffect(() => {
    const fetchVouchers = async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/api/promotions/my-vouchers`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
    ...
    <div className="space-y-8 animate-fadeIn">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[12px] text-[#88694f] font-bold italic">
            Danh sách các mã giảm giá bạn đã sưu tầm
          </p>
        </div>
        <div className="bg-[#800a0d]/5 px-4 py-2 rounded-2xl border border-[#800a0d]/10">
          <span className="text-[10px] font-black text-[#800a0d] uppercase tracking-widest">
            Tổng cộng: {vouchers.length}
          </span>
        </div>
      </div>
      ...
      {vouchers.length === 0 ? (
        <div className="bg-white p-20 text-center rounded-[40px] border-2 border-dashed border-gray-100">
          ...
          <p className="max-w-xs mx-auto text-sm font-bold leading-relaxed text-gray-400 italic">
            Bạn chưa có voucher nào. Hãy tham gia vòng quay may mắn hoặc các
            chương trình mini game để nhận thưởng nhé!
          </p>
        </div>
      ) : (
        <>
                ...
                <div className="w-24 bg-[#800a0d] flex flex-col items-center justify-center p-2 relative">
                  <div className="absolute top-0 bottom-0 left-[-8px] flex flex-col justify-between py-2">
                    {[...Array(6)].map((_, i) => (
                      <div
                        key={i}
                        className="w-4 h-4 rounded-full bg-[#fdfaf5]"
                      ></div>
                    ))}
                  </div>
                  <Ticket size={32} className="text-white/30 mb-2" />
                  <span className="text-white font-black text-[10px] uppercase tracking-widest vertical-text">
                    Voucher
                  </span>
                </div>
              ...
              {[...Array(totalPages)].map((_, i) => (
                <button
                  ...
                  className={`w-10 h-10 rounded-full font-black text-xs transition-all ${
                    currentPage === i + 1
                      ? "bg-[#800a0d] text-white shadow-lg"
                      : "bg-white text-[#88694f] border border-gray-100"
                  }`}
                >
                  {i + 1}
                </button>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#fdfaf5] w-full max-w-lg rounded-[40px] shadow-2xl overflow-hidden animate-zoomIn border-2 border-[#800a0d]">
            <div className="bg-[#800a0d] p-8 text-center text-white relative">
              <button
                onClick={() => setIsModalOpen(false)}
                className="absolute top-6 right-6 p-2 rounded-full bg-white/20 hover:bg-white/40"
              >
                <X size={20} />
              </button>
              ...
              <p className="text-xs font-bold opacity-80 uppercase tracking-widest">
                Mã voucher: {selectedVoucher.code}
              </p>
            </div>
            ...
            <div className="p-10 space-y-8">
              <div className="space-y-4">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h5 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">
                      Điều kiện áp dụng
                    </h5>
                    <p className="text-sm font-bold text-[#3e2714] leading-relaxed italic border-l-2 border-[#f39200] pl-3">
                      {selectedVoucher.description ||
                        "Áp dụng cho mọi đơn hàng tại hệ thống ClickGo."}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white p-4 rounded-[24px] border border-gray-100 shadow-sm">
                    ...
                    <p className="text-base font-black text-[#800a0d]">
                      {selectedVoucher.minOrderValue?.toLocaleString() || 0}đ
                    </p>
                  </div>
                  <div className="bg-white p-4 rounded-[24px] border border-gray-100 shadow-sm">
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">
