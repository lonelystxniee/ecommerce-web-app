...
const AccountPage = () => {
  ...
  return (
    <div className="min-h-screen pt-4 pb-20 font-sans bg-transparent">
              ...
              <div className="relative z-10">
                <div className="inline-block p-1 mb-4 bg-white rounded-full shadow-lg">
                  <div className="w-20 h-20 rounded-full bg-[#fdfaf5] flex items-center justify-center text-[#800a0d] text-2xl font-black border-2 border-[#800a0d]/10 overflow-hidden">
                    {user.avatar ? <img src={user.avatar} className="object-cover w-full h-full" alt="Avatar" /> : user.fullName?.charAt(0).toUpperCase()}
                  </div>
                </div>
                <h3 className="text-lg font-bold tracking-tight text-white uppercase truncate">Chào, {user.fullName?.split(' ').pop()}</h3>
                <p className="text-[11px] text-white/70 font-medium italic">Thành viên ClickGo</p>
              </div>
            </div>

            <nav className="p-3 space-y-1">
              <SidebarItem icon={<User size={18} />} text="Thông tin tài khoản" active={activeTab === 'info'} onClick={() => navigate('/account?tab=info')} />
              <SidebarItem icon={<Package size={18} />} text="Đơn hàng của tôi" active={activeTab === 'orders'} onClick={() => navigate('/account?tab=orders')} />
              <SidebarItem icon={<Heart size={18} />} text="Sản phẩm yêu thích" active={activeTab === 'favorites'} onClick={() => navigate('/account?tab=favorites')} />
              <SidebarItem icon={<Lock size={18} />} text="Đổi mật khẩu" active={activeTab === 'password'} onClick={() => navigate('/account?tab=password')} />
...
const SidebarItem = ({ icon, text, active, onClick }) => (
  <button
    onClick={onClick}
    className={`flex items-center gap-3 w-full px-5 py-3.5 transition-all duration-200 rounded-xl text-[15px] font-bold  tracking-tight
  ...
  return (
    <div className="p-8 bg-white border border-gray-100 shadow-2xl animate-zoomIn rounded-3xl">
      <div className="flex items-center justify-between pb-6 mb-8 border-b border-gray-200 border-dashed">
        <h2 className="text-2xl font-black text-[#800a0d]  tracking-tighter">Hồ sơ của tôi</h2>
        <button
          onClick={() => {
            if (isEditing) {
      ...
      <div className="flex flex-col gap-12 lg:flex-row">
        ...
        <div className="flex flex-col items-center shrink-0">
          <div className="relative">
            <div className="w-44 h-44 rounded-full p-1.5 border-2 border-dashed border-[#800a0d]/30 relative group">
              <div className="w-full h-full overflow-hidden bg-gray-50 rounded-full flex items-center justify-center text-[#800a0d] text-5xl font-black shadow-inner">
                {avatar ? <img src={avatar} alt="Avatar" className="object-cover w-full h-full transition-transform duration-500 group-hover:scale-110" /> : user.fullName.charAt(0).toUpperCase()}
              </div>
          ...
          <p className="mt-4 text-[10px] font-black text-[#88694f] uppercase tracking-[0.2em]">Ảnh đại diện</p>
        </div>
        ...
        <div className="flex-1 space-y-8">
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
            <div className="space-y-2">
              <label className="block pl-1 text-[13px] font-black text-text-primary">Họ và tên</label>
              <input
                type="text"
                name="fullName"
                disabled={!isEditing}
            ...
            <div className="space-y-2">
              <label className="text-[13px] font-black text-text-primary block pl-1">Số điện thoại</label>
              <input
                type="text"
                name="phone"
                disabled={!isEditing}
          ...
          <div className="space-y-2">
            <label className="text-[13px] font-black text-text-primary block pl-1">Địa chỉ email</label>
            <input type="email" disabled value={user.email} className="w-full px-5 py-2 font-bold text-gray-400 border border-gray-100 cursor-not-allowed bg-gray-50 rounded-2xl" />
          </div>

          <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
            <div className="space-y-2">
              <label className="text-[13px] font-black text-text-primary block pl-1">Giới tính</label>
              <select
                name="gender"
                disabled={!isEditing}
            ...
            <div className="space-y-2">
              <label className="text-[13px] font-black text-text-primary block pl-1">Ngày sinh</label>
              <input
                type="date"
                name="birthday"
                disabled={!isEditing}
...
const OrderList = ({ userId }) => {
  ...
  const handleCancelOrder = async (reason) => {
    ...
    try {
      const token = localStorage.getItem('token')
      const res = await fetch(`${API_URL}/api/orders/cancel/${orderToCancel._id}`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      toast.error('Lỗi kết nối máy chủ', e.message)
    }
  }
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter((order) => {
      ...
      const matchSearch = order._id.toLowerCase().includes(searchTerm.toLowerCase()) || order.customerInfo.fullName.toLowerCase().includes(searchTerm.toLowerCase())
  ...
  if (loading) return <div className="p-20 text-center font-bold text-[#88694f] animate-pulse">Đang tải lịch sử đơn hàng...</div>
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="bg-white p-6 rounded-[32px] shadow-xl border border-gray-100 space-y-6 print:hidden">
        <div className="flex flex-col justify-between gap-4 lg:flex-row">
          <h2 className="text-2xl font-black text-[#800a0d] uppercase tracking-tighter">Đơn hàng của tôi</h2>
          <div className="relative flex-1 max-w-md">
            <Search className="absolute -translate-y-1/2 left-4 top-1/2 text-stone-400" size={18} />
            <input
              type="text"
          ...
          {Object.keys(STATUS_LABELS).map((st) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                statusFilter === st ? 'bg-[#800a0d] text-white shadow-lg scale-105' : 'bg-gray-50 text-gray-400 hover:bg-gray-100'
              }`}
            >
              {STATUS_LABELS[st]}
            </button>
      ...
      {currentItems.length === 0 ? (
        <div className="bg-white p-20 text-center rounded-[40px] border-2 border-dashed border-gray-200">
          ...
          <p className="italic font-bold text-gray-400">Không tìm thấy đơn hàng nào phù hợp.</p>
        </div>
      ) : (
            ...
            >
              <div className="p-6 flex flex-col md:flex-row justify-between items-center gap-4 bg-[#fdfaf5]/50 group-hover:bg-[#fdfaf5]">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-black text-[#3e2714]">#{order._id.slice(-8).toUpperCase()}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase ${getStatusColor(order.status)}`}>{STATUS_LABELS[order.status] || order.status}</span>
                  ...
                  {['PENDING', 'CONFIRMED', 'PACKING'].includes((order.status || '').toUpperCase()) && !order.ghnOrderCode && (
                    <button
                      ...
                      className="px-4 py-2 bg-white text-red-500 rounded-xl border border-red-50 hover:bg-red-50 transition-all shadow-sm text-[10px] font-black uppercase tracking-widest flex items-center gap-2"
                      title="Hủy đơn hàng"
                    >
                      <X size={14} /> Hủy đơn
                    </button>
              ...
              <div className="flex items-center justify-between p-6 border-t border-gray-50">
                ...
                <div className="text-right">
                  <p className="text-[10px] font-bold text-gray-300 uppercase">Tổng thanh toán</p>
                  <p className="text-xl font-black text-[#800a0d]">{order.totalPrice.toLocaleString()}đ</p>
                </div>
              </div>
            </div>
          ))}
          ...
          <div className="relative bg-white w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-[40px] shadow-2xl flex flex-col border-t-8 border-[#800a0d]">
            <div className="sticky top-0 z-10 flex items-center justify-between p-6 bg-white border-b print:hidden">
              <div className="flex items-center gap-2 text-[#800a0d]">
                <Printer size={20} />
                <h3 className="font-black tracking-widest uppercase">Chi tiết hóa đơn</h3>
              </div>
              <button onClick={() => setIsModalOpen(false)} className="p-2 transition-all rounded-full hover:bg-gray-100">
                <X size={24} />
              </button>
            </div>
            ...
            <div ref={componentRef} className="p-8 space-y-10 bg-white md:p-12">
              ...
              <div className="flex justify-between items-start border-b-4 border-[#800a0d] pb-8">
                <div>
                  <h1 className="text-4xl font-black text-[#800a0d] tracking-tighter">CLICK GO</h1>
                  <p className="text-sm italic font-bold text-gray-500">Tinh hoa quà Việt - Hệ thống ClickGo</p>
                </div>
                <div className="text-right">
                  <h2 className="text-2xl font-black text-[#800a0d] uppercase">Hóa đơn bán hàng</h2>
                  <p className="text-sm font-bold">Số: #{selectedOrder._id.slice(-12).toUpperCase()}</p>
                  <p className="text-[10px] text-gray-400 italic">Ngày in: {new Date().toLocaleString('vi-VN')}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-12 text-sm">
                <div className="space-y-3">
                  <h4 className="text-[10px] font-black uppercase text-[#800a0d] border-b border-gray-100 pb-2">Thông tin người nhận</h4>
                  <p className="text-base font-black text-gray-800">{selectedOrder.customerInfo.fullName}</p>
                  <p className="font-bold text-gray-500">SĐT: {selectedOrder.customerInfo.phone}</p>
                  <p className="text-xs italic leading-relaxed text-gray-400">{selectedOrder.customerInfo.address}</p>
                </div>
                <div className="space-y-3">
                  <h4 className="text-[10px] font-black uppercase text-[#800a0d] border-b border-gray-100 pb-2">Chi tiết giao dịch</h4>
                  <p className="font-bold text-gray-700">
                    PT Thanh toán: <span className="text-orange-600">{selectedOrder.paymentMethod}</span>
                  </p>
                  <p className="text-xs font-bold uppercase">Trạng thái: {STATUS_LABELS[selectedOrder.status]}</p>
                  <p className="text-xs italic font-bold text-gray-400">Ngày đặt: {new Date(selectedOrder.createdAt).toLocaleString('vi-VN')}</p>
                </div>
              </div>
                    ...
                    {selectedOrder.items.map((item, idx) => (
                      <tr key={idx}>
                        <td className="py-4 pl-6">
                          <div className="flex items-center gap-3">
                            <img src={item.image || (item.images && item.images[0])} className="object-contain w-12 h-12 rounded-lg bg-gray-50 print:hidden" alt="" />
                            <div>
                              <p className="font-black text-gray-800">{item.name}</p>
                              <p className="text-[10px] text-gray-400 italic">{item.label || 'Tiêu chuẩn'}</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-4 font-black text-center">x{item.quantity}</td>
                        <td className="py-4 pr-6 font-black text-right text-gray-800">{item.price.toLocaleString()}đ</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                <div className="p-8 bg-[#800a0d]/5 flex justify-between items-center print:bg-white print:border-t-2">
                  <span className="font-black text-[#800a0d] uppercase tracking-widest text-xs print:text-black">Tổng thanh toán</span>
                  <span className="text-4xl font-black text-[#800a0d] print:text-black">{selectedOrder.totalPrice.toLocaleString()}đ</span>
                </div>
              </div>
              ...
              {['PENDING', 'CONFIRMED', 'PACKING'].includes((selectedOrder.status || '').toUpperCase()) && !selectedOrder.ghnOrderCode && (
                <div className="pt-4 border-t border-gray-100 border-dashed print:hidden">
                  <button
                    onClick={() => {
                      setOrderToCancel(selectedOrder)
                      setIsCancelModalOpen(true)
                    }}
                    className="flex items-center justify-center w-full gap-2 py-4 font-bold text-red-600 transition-all rounded-3xl bg-red-50 hover:bg-red-100 active:scale-95"
                  >
                    <X size={18} /> Hủy đơn hàng ngay
                  </button>
                </div>
              )}
            </div>
            ...
            <div className="sticky bottom-0 z-10 flex flex-col justify-center gap-4 p-8 border-t bg-gray-50 md:flex-row print:hidden">
              ...
              <button onClick={() => setIsModalOpen(false)} className="bg-[#3e2714] text-white px-12 py-3 rounded-full font-black uppercase text-xs tracking-widest hover:bg-black transition-all">
                Đóng
              </button>
            </div>
          </div>
      ...
      <CancellationModal isOpen={isCancelModalOpen} onClose={() => setIsCancelModalOpen(false)} onConfirm={handleCancelOrder} orderId={orderToCancel?._id} />
    </div>
  )
}
  ...
  return (
    <div className="p-8 mx-auto bg-white border border-gray-100 shadow-2xl animate-zoomIn rounded-3xl">
      <div className="flex items-center gap-4 pb-6 mb-8 border-b border-gray-200 border-dashed">
        ...
        <div>
          <h2 className="text-2xl font-black text-[#800a0d] tracking-tighter">Đổi mật khẩu</h2>
          <p className="text-xs font-medium text-gray-400">Cập nhật mật khẩu để bảo vệ tài khoản của bạn</p>
        </div>
      </div>
      ...
      <form onSubmit={handleChangePassword} className="space-y-6">
        {[
          ...
          <div key={name} className="space-y-2">
            <label className="block pl-1 text-[13px] font-black text-text-primary">{label}</label>
            <input
              type="password"
              name={name}
              required
              ...
              className={`w-full px-5 py-2 bg-[#fdfaf5] border rounded-2xl outline-none focus:bg-white transition-all font-bold text-[#3e2714] ${
                errors[name] ? 'border-red-500' : 'border-gray-100 focus:border-[#800a0d]'
              }`}
            />
            {errors[name] && <p className="pl-1 text-xs font-bold text-red-500">{errors[name]}</p>}
          </div>
        ))}
        <div className="pt-4">
          <button
...
const ActivityHistory = () => {
  ...
  useEffect(() => {
    const fetchActivities = async () => {
      try {
        const token = localStorage.getItem('token')
        const res = await fetch(`${import.meta.env.VITE_API_URL || ''}/api/auth/activities`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        const data = await res.json()
  ...
  const filteredActivities = useMemo(() => {
    return activities.filter((activity) => {
      const matchAction = filterAction === 'ALL' || activity.action === filterAction
      const matchSearch = (activity.action || '').toLowerCase().includes(searchTerm.toLowerCase()) || (activity.details || '').toLowerCase().includes(searchTerm.toLowerCase())
      ...
      if (filterDate !== 'ALL') {
        ...
        const now = new Date()
        if (filterDate === 'TODAY') {
          matchDate = activityDate.toDateString() === now.toDateString()
        } else if (filterDate === 'WEEK') {
          const weekAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000)
          matchDate = activityDate >= weekAgo
        } else if (filterDate === 'MONTH') {
          const monthAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000)
          matchDate = activityDate >= monthAgo
        }
      }

      return matchAction && matchSearch && matchDate
    })
  }, [activities, filterAction, searchTerm, filterDate])
  ...
  if (loading)
    return (
      <div className="flex flex-col items-center justify-center p-20 bg-white shadow-2xl rounded-3xl animate-pulse">
        ...
        <span className="font-bold text-[#88694f]">Đang tải lịch sử hoạt động...</span>
      </div>
    )
  ...
  return (
    <div className="p-8 bg-white border border-gray-100 shadow-2xl rounded-[40px] animate-fadeIn">
      <div className="flex items-center gap-4 pb-6 mb-8 border-b border-gray-200 border-dashed">
        ...
        <div>
          <h2 className="text-2xl font-black text-[#800a0d] tracking-tighter">Lịch sử hoạt động</h2>
          <p className="text-xs font-medium text-gray-400">Các hoạt động gần đây trên tài khoản của bạn</p>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-4 mb-8 sm:flex-row">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 bg-[#fdfaf5] border border-gray-100 rounded-2xl text-sm focus:ring-2 focus:ring-[#800a0d] outline-none transition-all font-bold text-[#3e2714]"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
          </div>
          <select
            ...
            className="w-full sm:w-40 px-4 py-3 bg-[#fdfaf5] border border-gray-100 rounded-2xl outline-none focus:ring-2 focus:ring-[#800a0d] transition-all font-bold text-[#3e2714] appearance-none cursor-pointer"
          >
          ...
          <select
            ...
            className="w-full sm:w-48 px-4 py-3 bg-[#fdfaf5] border border-gray-100 rounded-2xl outline-none focus:ring-2 focus:ring-[#800a0d] transition-all font-bold text-[#3e2714] appearance-none cursor-pointer"
          >
            {uniqueActions.map((action) => (
              <option key={action} value={action}>
                {action === 'ALL' ? 'Tất cả hoạt động' : action}
              </option>
            ))}
          </select>
        </div>
      ...
      {currentItems.length === 0 ? (
        <div className="py-20 text-center bg-[#fdfaf5]/50 rounded-3xl border border-dashed border-gray-200">
          <div className="flex items-center justify-center w-20 h-20 mx-auto mb-4 text-gray-300 bg-white border rounded-full shadow-sm border-gray-50">
            <History size={32} />
          </div>
          <p className="text-sm italic font-bold text-gray-400">{activities.length === 0 ? 'Chưa có hoạt động nào được ghi lại' : 'Không tìm thấy hoạt động phù hợp'}</p>
        </div>
      ) : (
            ...
            <div key={activity._id} className="relative pl-14 group">
              ...
              <div className="bg-[#fdfaf5]/30 p-5 rounded-3xl border border-gray-100 transition-all hover:bg-white hover:shadow-xl hover:border-[#800a0d]/10">
                <div className="flex flex-col items-start justify-between gap-2 mb-2 sm:flex-row">
                  <h4 className="font-black text-[#800a0d] text-base tracking-tight">{activity.action}</h4>
                ...
                <p className="text-sm font-medium leading-relaxed text-gray-600">{activity.details}</p>
                {activity.ipAddress && (
          ...
          {[...Array(totalPages)].map((_, i) => (
            <button
              ...
              className={`w-9 h-9 rounded-full font-black text-xs transition-all ${
                currentPage === i + 1 ? 'bg-[#800a0d] text-white shadow-lg scale-110' : 'bg-white text-[#88694f] border border-gray-100 hover:border-[#800a0d]'
              }`}
            >
              {i + 1}
            </button>
  ...
  if (wishlistItems.length === 0) {
    return (
      <div className="p-12 text-center bg-white border border-gray-100 shadow-2xl rounded-[40px] animate-fadeIn">
        ...
        <h2 className="text-2xl font-black text-[#3e2714] mb-2 tracking-tight">Danh sách yêu thích trống</h2>
        <p className="max-w-xs mx-auto mb-8 text-sm italic text-gray-400">Lưu lại những món quà bạn yêu thích để dễ dàng tìm lại sau này.</p>
        <Link
          to="/"
    ...
    <div className="bg-white p-8 border border-gray-100 shadow-2xl rounded-[40px] animate-fadeIn">
      ...
      <div className="flex items-center justify-between pb-6 mb-8 border-b border-gray-200 border-dashed">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-black text-[#800a0d] tracking-tighter">Sản phẩm yêu thích</h2>
            <p className="text-xs font-bold text-gray-400">{wishlistItems.length} sản phẩm đang được lưu</p>
          </div>
        </div>
        <Link to="/wishlist" className="text-[11px] font-black text-[#88694f] hover:text-[#800a0d] uppercase tracking-widest transition-colors flex items-center gap-1">
      ...
      <div className="grid grid-cols-2 gap-4 mb-8 sm:grid-cols-3">
        {currentItems.map((item) => {
          ...
          return (
            <div key={item._id} className="group bg-[#fdfaf5]/50 border border-gray-100 rounded-[24px] overflow-hidden hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300">
              <Link to={`/product/${item._id}`} className="relative block overflow-hidden bg-white aspect-square">
                <img src={displayImage} alt={item.productName} className="object-contain w-full h-full p-2 transition-transform duration-500 group-hover:scale-110" />
                <button
                  onClick={(e) => {
                    e.preventDefault()
                    toggleWishlist(item._id)
                  }}
                  className="absolute flex items-center justify-center text-red-400 transition-all scale-75 bg-white rounded-full shadow-md opacity-0 top-2 right-2 w-7 h-7 hover:bg-red-500 hover:text-white group-hover:opacity-100 group-hover:scale-100"
              ...
              <div className="p-3">
                <Link to={`/product/${item._id}`} className="block text-sm font-bold text-[#3e2714] hover:text-[#800a0d] transition-colors line-clamp-2 mb-1 leading-snug">
                  {item.productName}
                </Link>
                <p className="text-xs font-black text-[#800a0d] mb-3">{item.price?.toLocaleString()}đ</p>
                <button
        ...
        <div className="flex items-center justify-center gap-2 pt-2">
          ...
          {[...Array(totalPages)].map((_, i) => (
            <button
              ...
              onClick={() => setCurrentPage(i + 1)}
              className={`w-9 h-9 rounded-full font-black text-xs transition-all ${
                currentPage === i + 1 ? 'bg-[#800a0d] text-white shadow-lg' : 'bg-white text-[#88694f] border border-gray-100 hover:border-[#800a0d]'
              }`}
            >
              {i + 1}
            </button>
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      {vouchers.length === 0 ? (
        <div className="bg-white p-20 text-center rounded-[40px] border-2 border-dashed border-gray-100">
          ...
          <p className="max-w-xs mx-auto text-sm italic font-bold leading-relaxed text-gray-400">
            ...
            {currentItems.map((v) => (
              ...
              >
                ...
                <div className="w-24 bg-[#800a0d] flex flex-col items-center justify-center p-2 relative">
                  ...
                  <Ticket size={32} className="mb-2 text-white/30" />
                ...
                <div className="flex-1 p-5 relative overflow-hidden bg-gradient-to-br from-white to-[#fdfaf5]/30">
                  <div className="flex flex-col h-full">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <h4 className="text-base font-black text-[#800a0d] line-clamp-1">{v.discountType === 'PERCENT' ? `Giảm ${v.discountValue}%` : `Giảm ${v.discountValue.toLocaleString()}đ`}</h4>
                    ...
                    <div className="flex items-center justify-between pt-4 mt-auto border-t border-gray-200 border-dashed">
                      <code className="text-xs font-black text-[#3e2714] bg-white px-3 py-1.5 rounded-lg border border-gray-100 group-hover:border-[#800a0d] transition-all">{v.code}</code>
                      <button
                        onClick={(e) => {
                          e.stopPropagation()
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#fdfaf5] w-full max-w-lg rounded-[40px] shadow-2xl overflow-hidden animate-zoomIn border-2 border-[#800a0d]">
            <div className="bg-[#800a0d] p-8 text-center text-white relative">
              <button onClick={() => setIsModalOpen(false)} className="absolute p-2 rounded-full top-6 right-6 bg-white/20 hover:bg-white/40">
                <X size={20} />
              </button>
              <Ticket size={48} className="mx-auto mb-4 opacity-30" />
              <h3 className="mb-1 text-3xl font-black tracking-tighter uppercase">
              ...
              <p className="text-xs font-bold tracking-widest uppercase opacity-80">Mã voucher: {selectedVoucher.code}</p>
            </div>
            ...
            <div className="p-10 space-y-8">
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button onClick={() => setIsModalOpen(false)} className="px-8 py-4 bg-[#3e2714] text-white rounded-[24px] font-black text-xs uppercase tracking-widest hover:rounded-sm transition-all">
                  Đóng
                </button>
              </div>
            </div>
