...
// --- TAB 2: DANH SÁCH ĐƠN HÀNG (QUAN TRỌNG) ---
            ...
            >
              <div className="p-6 flex flex-col md:flex-row justify-between items-center gap-4 bg-[#fdfaf5]/50 group-hover:bg-[#fdfaf5]">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {/* NÚT THEO DÕI ĐƠN HÀNG (LOGO XE TẢI) */}
                  <Link
...
const FavoriteProducts = () => {
  ...
  return (
    <div className="bg-white p-8 border border-gray-100 shadow-2xl rounded-[40px] animate-fadeIn">
      <h2 className="text-2xl font-black text-[#800a0d] uppercase tracking-tighter mb-8 pb-6 border-b border-gray-200 border-dashed">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {wishlistItems.map((item) => {
          ...
          return (
            <div
              key={item._id}
              className="group bg-[#fdfaf5]/30 border border-gray-100 rounded-3xl overflow-hidden hover:shadow-xl transition-all"
            >
              <Link to={`/product/${item._id}`} className="block relative aspect-square overflow-hidden bg-white">
                <img
                  src={displayImage}
              ...
              <div className="p-5">
                <Link to={`/product/${item._id}`} className="text-base font-bold text-[#3e2714] hover:text-[#800a0d] transition-colors line-clamp-1 mb-1">
                  {item.productName}
                </Link>
                <p className="text-xs text-[#88694f] italic mb-4">
                  {item.price?.toLocaleString()}đ
                </p>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 bg-[#f39200] text-white py-2 rounded-xl font-bold text-[10px] uppercase tracking-wider hover:bg-orange-600 transition-colors flex items-center justify-center gap-1.5"
                  >
                    <ShoppingBasket size={14} /> Thêm giỏ
                  </button>
                  <button
