...
const LuckyWheel = () => {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      if (dataPromos.success)
        setDbPromos(dataPromos.promos.filter((p) => p.status === "ACTIVE"));
    ...
    const formattedPromos = dbPromos.map((p) => ({
      ...
      description:
        p.discountType === "AMOUNT"
          ? `Giảm ${p.discountValue.toLocaleString()}đ`
  ...
  const spinWheel = async () => {
    ...
    setTimeout(async () => {
      ...
      if (winner.code) {
        setMyVouchers((prev) => [
          winner,
          ...prev.filter((v) => v.code !== winner.code),
        ]);
        setShowPopup(true);
        try {
  ...
  return (
    <div
      className="min-h-screen bg-[#f7f4ef] font-sans pb-20 relative"
      style={{
    ...
    >
      <div className="max-w-[1200px] mx-auto px-4 pt-10">
        ...
        <div className="absolute top-10 right-10 bg-white p-4 rounded-3xl shadow-xl border-2 border-[#faa519]/30 flex items-center gap-4 animate-fadeIn z-50">
          ...
          <div>
            <p className="text-[10px] font-bold text-[#88694f] uppercase tracking-widest">
              Lượt quay
            </p>
            <p className="text-2xl font-black text-[#3e2714] leading-none">
              {availableSpins}
            </p>
          </div>
        </div>
          ...
          <div className="w-full max-w-md space-y-6">
            <div className="bg-white p-8 rounded-[40px] shadow-xl border-2 border-[#faa519]/20">
              ...
              <button
                onClick={() =>
                  canPlayMiniGame
                    ? setShowModal(true)
                    : toast.error("Hôm nay bạn đã chơi rồi!")
                }
            ...
            <div className="bg-[#3e2714] p-6 rounded-[32px] text-white shadow-lg border border-white/5 min-h-[320px]">
              ...
              <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {myVouchers.length === 0 ? (
                  <div className="py-10 text-center opacity-30 flex flex-col items-center text-xs italic text-white/50">
                  ...
                  myVouchers.map((v, idx) => (
                    <div
                      key={idx}
                      className="bg-white rounded-2xl p-4 flex items-center justify-between animate-fadeIn"
                    >
                ...
                <div className="grid gap-4">
                  ...
                  >
                    ...
                    <div className="text-left">
                      <p className="font-black text-[#3e2714] uppercase text-sm">
                        Chọn đáp án
                      </p>
                  ...
                  <button
                    ...
                    className="p-6 rounded-3xl border-2 border-gray-100 hover:border-[#9d0b0f] hover:bg-red-50 transition-all flex items-center gap-4 group"
                  >
                ...
                <div className="text-center space-y-6">
                  {gameStatus === "idle" ? (
                    <button
                      ...
                      className="w-full bg-[#9d0b0f] text-white py-4 rounded-2xl font-black uppercase"
                    >
                      Bắt đầu
                    </button>
                  ) : gameStatus === "playing" ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <p className="font-black text-xl text-[#3e2714]">
                        THÀNH CÔNG!
                      </p>
                      <button
                        onClick={resetAllGames}
                        className="w-full bg-[#3e2714] text-white py-4 rounded-2xl font-black uppercase text-xs"
                      >
                        Quay ngay
                      </button>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <button
                        onClick={startClickGame}
                        className="w-full bg-[#9d0b0f] text-white py-4 rounded-2xl font-black uppercase text-xs"
                      >
                        Thử lại
                      </button>
                    </div>
                  ...
                  {gameStatus === "won" ? (
                    ...
                    <div className="space-y-4">
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                        Câu {currentQuestion + 1} / 2
                      </p>
                      <p className="text-lg font-black text-[#3e2714]">
