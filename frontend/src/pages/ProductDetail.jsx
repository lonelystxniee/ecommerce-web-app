...
import { Minus, Plus, Star, Phone, ShoppingCart, MessageSquare, User, Send, Share2, Copy, Camera, Video, Trash2, Play, X, Heart, ChevronRight, Facebook } from 'lucide-react'
...
const ProductDetail = () => {
    ...
    const fetchRelatedProducts = async () => {
      ...
      try {
        ...
        if (data.success) {
          ...
          const filtered = data.products.filter((p) => p._id !== id && p.quantity > 0).slice(0, 5)
    ...
    try {
      ...
      if (data.success) {
        ...
        if (savedUser) {
          ...
          const existingReview = data.reviews.find((r) => (r.userID && r.userID._id === currentUser._id) || r.userID === currentUser._id)
          if (existingReview) {
  ...
  const VideoModal = ({ videoUrl, onClose }) => {
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center p-4 duration-300 z-1000 bg-black/90 backdrop-blur-sm animate-in fade-in" onClick={onClose}>
        <button onClick={onClose} className="absolute text-white transition-all top-6 right-6 hover:scale-125 hover:rotate-90">
          <X size={40} />
        </button>
        <div className="relative w-full max-w-5xl overflow-hidden duration-300 bg-black shadow-2xl aspect-video rounded-2xl animate-in zoom-in" onClick={(e) => e.stopPropagation()}>
  ...
  const variants =
    product?.variants && product.variants.length > 0
      ...
      : product
        ? [{ label: 'Giá gốc', price: product.price, stock: product.quantity || 0 }].filter((v) => v.stock > 0)
        : []
  ...
  const handleShareFacebook = (e) => {
    ...
    window.open(url, 'facebook-share-dialog', `width=${width},height=${height},top=${top},left=${left},toolbar=0,status=0,menubar=0`)
  }

  const calculateTotal = () => quantities.reduce((total, q, i) => total + q * (variants[i]?.price || 0), 0)
  ...
  if (!product) return <div className="min-h-screen p-20 font-bold text-center bg-transparent text-section">Sản phẩm không tồn tại!</div>
    ...
    <div className="bg-transparent min-h-screen font-sans text-[#3e2714] pb-10">
      <div className="container px-4 py-6 mx-auto max-w-300">
        ...
        <div className="text-[12px] text-gray-500 mb-6 uppercase font-bold">
          Trang chủ / Sản phẩm / <span className="text-primary">{product.name}</span>
        </div>
        ...
        <div className="flex flex-col items-start gap-8 mb-12 lg:flex-row">
          ...
          <div className="flex flex-col w-full gap-4 lg:w-5/12">
            <div className="relative p-2 overflow-hidden bg-white border border-gray-100 rounded-sm shadow-md">
              <div onClick={prevImg} className="absolute w-8 h-8 -translate-y-1/2 cursor-pointer top-1/2 left-2 z-3">
                <img alt="prev" src="https://honglam.vn/_next/static/media/slick-prev-xs-hover.d0444fb5.png" className="w-8 h-8" />
              </div>
              <div onClick={nextImg} className="absolute w-8 h-8 -translate-y-1/2 cursor-pointer top-1/2 right-2 z-3">
                <img alt="next" src="https://honglam.vn/_next/static/media/slick-next-xs-hover.c14e777f.png" className="w-8 h-8" />
              </div>
              <div className="flex justify-center overflow-hidden bg-white">
                <img src={productImages[activeImg]} className="object-cover w-full h-100" alt={product.name} />
              </div>
            </div>
          ...
          <div className="flex flex-col w-full lg:w-4/12">
            <h1 className="mb-1 text-3xl font-bold tracking-tight uppercase font-seagull text-primary">{product.name}</h1>
            <p className="text-sm text-[#88694f] mb-4 italic">{product.slogan || 'Tinh hoa quà Việt'}</p>
            <div className="flex gap-1 mb-5">
              {[1, 2, 3, 4, 5].map((s) => (
                <Star key={s} size={14} fill="#faa519" className="text-secondary" />
              ))}
            </div>
            <p className="text-[13px] leading-relaxed text-gray-600 mb-8 border-b border-dashed border-gray-300 pb-6 italic">
              {product.description || 'Sản phẩm được chế biến theo quy trình nghiêm ngặt, giữ trọn hương vị tự nhiên đặc trưng của đặc sản Hà Thành.'}
            </p>
            ...
            <div className="bg-[#efe7db] rounded-lg p-1 shadow-inner overflow-hidden">
              {variants.length > 0 ? (
                <table className="w-full text-[13px]">
                  <thead>
                    <tr className="text-[#88694f] font-bold border-b border-gray-300">
                      <th className="py-2 text-left pl-3 uppercase text-[10px]">Phân loại</th>
                      <th className="py-2 text-center uppercase text-[10px]">Giá</th>
                      <th className="py-2 text-center uppercase text-[10px]">Còn lại</th>
                      <th className="py-2 text-center uppercase text-[10px]">Số lượng</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200">
                    {variants.map((v, i) => (
                      <tr key={i} className="font-bold hover:bg-white/30">
                        ...
                        <td className="py-4 text-center text-primary">{v.price.toLocaleString()}đ</td>
                        <td className="py-4 font-medium text-center text-gray-700">{v.stock}</td>
                        <td className="flex justify-center py-4">
                          <div className="flex items-center overflow-hidden bg-white border border-gray-300 rounded">
                            <button onClick={() => updateQty(i, -1)} className="px-2 py-1 text-gray-400 border-r">
                              <Minus size={12} />
                            </button>
                            <input
                              type="number"
                            ...
                            />
                            <button onClick={() => updateQty(i, 1)} className="px-2 py-1 text-gray-400 border-l">
                              <Plus size={12} />
                            </button>
                          </div>
                        </td>
                ...
                <div className="p-6 text-lg font-bold text-center text-red-500 bg-white/50">Sản phẩm hiện tại đã hết hàng!</div>
              )}
            </div>
            ...
            <div className="mt-8">
              {variants.length > 0 && (
                <p className="mb-6 text-xl italic font-bold">
                  Thành tiền: <span className="text-2xl font-black text-primary">{calculateTotal().toLocaleString()}đ</span>
                </p>
              )}
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className={`px-4 py-4 rounded-md shadow-lg transition-all active:scale-95 border-2 ${
                    isInWishlist(product._id) ? 'bg-red-50 border-[#9d0b0f] text-[#9d0b0f]' : 'bg-white border-gray-200 text-gray-400 hover:text-[#9d0b0f] hover:border-[#9d0b0f]'
                  }`}
              ...
              {/* PHẦN CHIA SẺ */}
              <div className="pt-6 mt-8 border-t border-gray-200 border-dashed">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="flex items-center justify-center w-8 h-8 text-gray-500 transition-transform bg-gray-100 border border-gray-200 rounded-full shadow-sm hover:bg-gray-200 hover:scale-110"
        ...
        <div className="p-8 bg-white border border-gray-100 shadow-sm rounded-xl">
          <div className="flex items-center gap-3 pb-4 mb-8 border-b border-gray-100">
            ...
            <h3 className="text-xl font-bold uppercase">Đánh giá & Bình luận ({reviews.length})</h3>
          </div>

          {/* Form đánh giá */}
          <div className="mb-12">
            <h4 className="mb-4 text-sm font-bold text-gray-500 uppercase">{userReview ? 'Chỉnh sửa đánh giá của bạn' : 'Gửi đánh giá của bạn'}</h4>
            <form onSubmit={handleSubmitReview} className="space-y-6">
              <div className="flex items-center gap-4">
                <span className="text-sm font-medium">Đánh giá của bạn:</span>
                <div className="flex gap-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button key={star} type="button" onClick={() => setUserRating(star)} className="transition-transform hover:scale-125 focus:outline-none">
                      <Star size={24} fill={star <= userRating ? '#f39200' : 'none'} className={star <= userRating ? 'text-[#f39200]' : 'text-gray-300'} />
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-4 md:flex-row">
                <div className="flex-1">
                  <textarea
                    ...
                    className="w-full h-32 p-4 text-sm transition-all border border-gray-200 outline-none bg-gray-50 rounded-xl focus:border-primary focus:bg-white"
                  />
                </div>
              </div>
          ...
          <div className="space-y-8">
            {reviews.length === 0 ? (
              <div className="py-10 italic text-center text-gray-400">Chưa có đánh giá nào cho sản phẩm này. Hãy là người đầu tiên đánh giá!</div>
            ) : (
              reviews.map((review) => (
                <div key={review._id} className="flex gap-4 pb-8 border-b border-gray-100 last:border-0">
                  <div className="flex items-center justify-center w-12 h-12 bg-gray-100 rounded-full shrink-0">
                    {review.userID?.avatar ? <img src={review.userID.avatar} className="object-cover w-full h-full rounded-full" alt="avatar" /> : <User className="text-gray-400" />}
                  </div>
                  <div className="flex-1">
                    ...
                    <div className="flex gap-0.5 mb-2">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star key={s} size={12} fill={s <= review.rating ? '#f39200' : 'none'} className={s <= review.rating ? 'text-[#f39200]' : 'text-gray-300'} />
                      ))}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {review.images?.map((img, i) => (
                        <img
                          key={i}
                          src={img}
                          className="object-cover w-20 h-20 transition-transform rounded-lg cursor-pointer hover:scale-105"
                          alt="review-img"
                          onClick={() => window.open(img, '_blank')}
                        />
                      ))}
                      {review.videos?.map((video, i) => (
                        <div key={i} className="relative flex items-center justify-center w-24 h-24 overflow-hidden bg-black rounded-lg cursor-pointer group" onClick={() => setActiveVideo(video)}>
                          <video src={video} className="object-cover w-full h-full opacity-60" />
        ...
        {relatedProducts.length > 0 && (
          <div className="pt-12 mt-16 border-t border-gray-100">
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-2xl font-bold tracking-tighter uppercase text-primary">Sản phẩm cùng loại</h3>
              <Link to={`/category/${product.categoryID?.[0]?.slug || ''}`} className="text-sm font-bold text-[#88694f] hover:text-primary transition-colors flex items-center gap-1">
