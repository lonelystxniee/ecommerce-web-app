...
const AuthForm = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const errors =
      viewMode === 'login' ? validateLogin({ email, password }) : validateRegister(registerData);
    ...
    try {
      ...
      if (data.success) {
        toast.success(
          viewMode === 'login' ? 'Đăng nhập thành công!' : 'Đăng ký thành công! Hãy đăng nhập.',
        );
        if (viewMode === 'login') {
              ...
              <div className="space-y-4">
                ...
                {forgotStatus === 'success' ? (
                  ...
                  <>
                    ...
                    />
                    {forgotStatus === 'error' && (
                      <p className="m-0 text-sm text-center text-red-500">{forgotMessage}</p>
                    )}
                    <button
                      type="submit"
