...
export default function CombinedChatWidget() {
  ...
  const formatMessage = (text) => {
    ...
    let formatted = text.replace(
      /\*\*(.*?)\*\*/g,
      '<b class="font-black text-inherit">$1</b>',
    );
    ...
    try {
      ...
      if (data.success) {
        const conv = (data.conversations || []).find(
          (c) => c.conversationId === conversationIdSupport,
        );
  ...
  const fetchSupportMessages = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/chat/conversation/${encodeURIComponent(conversationIdSupport)}`,
      );
      const data = await res.json();
  ...
  const markSupportAsRead = async () => {
    ...
    try {
      await fetch(
        `${API_URL}/api/chat/conversation/${encodeURIComponent(conversationIdSupport)}/read`,
        { method: "PUT" },
      );
      setUnreadCount(0);
    } catch (err) {
  ...
  const fetchAIMessages = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/chat/conversation/${encodeURIComponent(conversationIdAI)}`,
      );
      const data = await res.json();
  ...
  useEffect(() => {
    ...
    // Join both rooms
    if (conversationIdSupport)
      s.emit("join_conversation", { conversationId: conversationIdSupport });
    if (conversationIdAI)
      s.emit("join_conversation", { conversationId: conversationIdAI });
        ...
        setMessagesSupport((prev) => {
          ...
          if (msg.clientId)
            filtered = filtered.filter(
              (m) => String(m.clientId) !== String(msg.clientId),
            );
    ...
    return () => {
      if (conversationIdSupport)
        s.emit("leave_conversation", { conversationId: conversationIdSupport });
      if (conversationIdAI)
        s.emit("leave_conversation", { conversationId: conversationIdAI });
  ...
  const handleSendSupport = (overrideText) => {
    const content = (
      overrideText !== undefined ? overrideText : textSupport
    ).trim();
    ...
    <div className="fixed right-6 bottom-6 z-[9999] flex flex-col items-end print:hidden">
      {!open && (
        ...
        >
          ...
          <span className="text-3xl leading-none transition-transform duration-500 group-hover:rotate-12">
            💬
          </span>
          {unreadCount > 0 && (
      ...
      {open && (
        <div className="mt-3 w-[400px] max-w-[calc(100vw-3rem)] text-sm animate-zoomIn transform transition-all duration-300 origin-bottom-right drop-shadow-2xl">
          <div className="bg-white/95 backdrop-blur-2xl border border-white/50 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.15)] overflow-hidden flex flex-col h-[650px] max-h-[85vh]">
            ...
            <div className="pt-8 px-8 pb-6 bg-white relative">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold flex items-center gap-1.5 mt-1">
                      <span
                        className={`w-2 h-2 rounded-full animate-pulse ${activeTab === "support" ? "bg-red-500" : "bg-teal-500"}`}
                      ></span>
                      <span
                        className={
                          activeTab === "support"
                            ? "text-red-500"
                            : "text-teal-600 uppercase tracking-tighter"
                        }
                      >
                        {activeTab === "support"
                          ? "Đang trực tuyến"
              ...
              <div className="flex p-1.5 bg-gray-100/80 rounded-[24px] border border-gray-100 relative">
                ...
                >
                  <span>HỖ TRỢ</span>
                  {unreadCount > 0 && (
                    <span className="bg-red-600 text-white text-[9px] px-2 py-0.5 rounded-full shadow-md">
                      {unreadCount}
                    </span>
                  )}
                </button>
                ...
                >
                  <span>TRỢ LÝ AI</span>
                  <span className="bg-teal-100/50 text-teal-600 text-[8px] px-1.5 py-0.5 rounded-md font-black tracking-tighter">
                    BETA
                  </span>
                </button>
              </div>
            ...
            <div
              ref={scrollRef}
              className="flex-1 p-8 overflow-y-auto space-y-6 bg-gradient-to-b from-white to-[#fcfaf7] custom-scrollbar scroll-smooth"
            >
              {activeTab === "support" ? (
                <>
                  {messagesSupport.length === 0 && (
                    <div className="h-full flex flex-col items-center justify-center text-center py-10 opacity-70">
                      ...
                      <h4 className="font-seagull text-lg text-gray-800 mb-2">
                        Chào {user.fullName || "bạn"}!
                      </h4>
                      <p className="text-gray-500 text-xs px-12 leading-relaxed">
                        Chúng tôi luôn ở đây để lắng nghe và giải đáp mọi thắc
                        mắc của bạn.
                      </p>
                    </div>
                  )}
                  {messagesSupport.map((m) => (
                    <div
                      key={m._id}
                      className={`flex ${m.senderRole === "admin" ? "justify-start" : "justify-end"} animate-zoomIn`}
                    >
                      <div
                      ...
                      >
                        ...
                        />
                        <div
                          className={`text-[9px] mt-2 font-bold opacity-60 uppercase tracking-widest ${m.senderRole === "admin" ? "text-gray-400" : "text-red-100 text-right"}`}
                        >
                ...
                <>
                  {messagesAI.length === 0 && !loadingAI && (
                    <div className="h-full flex flex-col items-center justify-center text-center py-10 opacity-70">
                      <div className="w-24 h-24 rounded-full bg-teal-50 flex items-center justify-center text-4xl mb-6 shadow-inner">
                        ✨
                      </div>
                      <h4 className="font-seagull text-lg text-gray-800 mb-2">
                        Trợ lý AI Momotrust
                      ...
                      <p className="text-gray-500 text-xs px-12 leading-relaxed">
                        Nhập mã sản phẩm hoặc câu hỏi về chính sách để được tư
                        vấn ngay lập tức!
                      </p>
                    </div>
                  )}
                  {messagesAI.map((m) => (
                    <div
                      key={m._id}
                      className={`flex ${m.senderRole === "ai" || m.isAI ? "justify-start" : "justify-end"} animate-zoomIn`}
                    >
                      <div
                      ...
                      >
                        ...
                        />
                        <div
                          className={`text-[9px] mt-2 font-bold opacity-60 uppercase tracking-widest ${m.senderRole === "ai" || m.isAI ? "text-teal-400" : "text-teal-50 text-right"}`}
                        >
                          {new Date(m.createdAt).toLocaleTimeString([], {
                            hour: "2-digit",
                            minute: "2-digit",
            ...
            <div className="p-8 bg-white/95 backdrop-blur-xl border-t border-gray-100 relative">
              ...
              >
                <input
                  ...
                  placeholder={
                    activeTab === "support"
                      ? "Gửi lời nhắn chuyên gia..."
                      : "Hỏi trợ lý AI về sản phẩm..."
                  }
                ...
                >
                  {activeTab === "ai" && loadingAI ? (
                    ...
                    <svg
                      ...
                      viewBox="0 0 24 24"
                      fill="currentColor"
                      className="w-6 h-6 transform rotate-45 group-hover:scale-110 active:rotate-12 transition-transform duration-300"
                    >
              ...
              <div className="flex items-center justify-center gap-2 mt-6">
                <span
                  className={`w-1 h-1 rounded-full ${activeTab === "support" ? "bg-red-400" : "bg-teal-400"}`}
                ></span>
                <p className="text-[10px] text-gray-400 font-black uppercase tracking-[0.2em] opacity-40">
                  {activeTab === "ai"
                    ? "MOMOTRUST INTELLIGENCE"
