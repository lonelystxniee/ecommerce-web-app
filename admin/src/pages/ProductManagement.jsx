...
const ProductManagement = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (imageFiles.length === 0 && !isEditMode) return alert('Vui lòng thêm ít nhất một ảnh sản phẩm!')

    setIsSubmitting(true)

    try {
      const url = isEditMode ? `${API_URL}/api/products/${editingProductId}` : `${API_URL}/api/products`
  ...
  const handleImportExcel = async (e) => {
    ...
    if (!window.confirm(`Bạn có chắc chắn muốn nhập sản phẩm từ tệp ${file.name}?`)) return
  ...
  const handleEdit = (product) => {
    setFormData({
      ...
      variants: product.variants && product.variants.length > 0 ? product.variants : [{ label: 'Giá gốc', price: product.price, stock: product.quantity }],
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex flex-col gap-4 border-b-2 border-[#9d0b0f] pb-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-black text-[#9d0b0f] uppercase tracking-tighter">Quản lý sản phẩm</h2>
          </div>
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 px-6 py-3 font-bold text-white transition-all bg-green-600 shadow-lg cursor-pointer rounded-2xl hover:bg-green-700">
        ...
        <div className="flex flex-wrap items-center gap-4 p-4 bg-gray-50 rounded-2xl">
          ...
          <div className="flex items-center gap-2 px-2 bg-white border-2 border-gray-100 rounded-xl">
            <input
              type="number"
              placeholder="Giá từ..."
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
        {products.map((p) => {
          const displayImage = p.images && p.images.length > 0 ? p.images[0] : p.image || 'https://via.placeholder.com/150'

          return (
            <div key={p._id} className="bg-white rounded-[32px] border border-gray-100 overflow-hidden hover:shadow-xl transition-all group">
              ...
              <div className="p-6">
                <h3 className="font-bold text-[#3e2714] line-clamp-1">{p.name}</h3>
                <p className="text-[#9d0b0f] text-lg font-black mt-2">Chỉ từ {p.price?.toLocaleString() || 0}đ</p>
                    ...
                    {p.categoryID && p.categoryID.length > 0 ? (
                      <>
                        ...
                        {p.categoryID.length > 3 && (
                          <span
                            className="text-[10px] bg-red-100 text-[#9d0b0f] px-1.5 py-1 rounded-lg font-bold uppercase"
                            title={p.categoryID
                              .slice(3)
                              .map((c) => c.name)
                              .join(', ')}
                      ...
                      <span className="text-[10px] bg-gray-50 text-gray-500 px-2 py-1 rounded-lg font-bold uppercase">Chưa phân loại</span>
                    )}
                  </div>
                  <div className="flex gap-1">
                    <button onClick={() => handleEdit(p)} className="p-2 text-blue-500 transition-all bg-blue-50 rounded-xl hover:bg-blue-500 hover:text-white">
                      <Edit2 size={16} />
                    </button>
                    <button onClick={() => handleDelete(p._id)} className="p-2 text-red-500 transition-all bg-red-50 rounded-xl hover:bg-red-500 hover:text-white">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </div>
      ...
      {pagination.totalPages > 1 && (
        <div className="flex flex-col items-center gap-4 pt-8 mt-8 border-t border-gray-200 border-dashed">
                ...
                p === '...' ? (
                  ...
                  <button
                    ...
                    className={`w-9 h-9 rounded-xl text-sm font-bold transition-all ${
                      currentPage === p ? 'bg-[#9d0b0f] text-white shadow-lg' : 'border border-gray-200 hover:border-[#9d0b0f] hover:text-[#9d0b0f]'
                    }`}
                  >
                    {p}
                  </button>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#f7f4ef] w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-[32px] shadow-2xl border-2 border-[#9d0b0f]">
            <div className="bg-[#9d0b0f] p-6 text-white flex justify-between items-center sticky top-0 z-10">
              <h3 className="text-xl font-bold uppercase">{isEditMode ? 'Cập nhật sản phẩm' : 'Thêm sản phẩm mới'}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-8">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-[10px] font-black uppercase text-[#88694f] mb-1.5 ml-1">Mã sản phẩm (SKU)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase text-[#88694f] mb-1.5 ml-1">Tên sản phẩm</label>
                  <input
                    type="text"
                    className="w-full bg-[#f7f4ef] border-2 border-stone-200 p-3.5 rounded-2xl outline-none focus:border-[#9d0b0f] transition-all font-bold text-[#3e2714]"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-black uppercase text-[#88694f] mb-1.5 ml-1">Danh mục(Có thể chọn nhiều)</label>
                    <div className="flex flex-col gap-2">
                      ...
                      {Array.isArray(formData.category) && formData.category.length > 0 && (
                        <div className="flex flex-wrap gap-2 p-3 mt-2 bg-white border-2 border-gray-200 border-dashed rounded-2xl">
                          {formData.category.map((catName) => (
                            <span key={catName} className="px-3 py-1.5 bg-red-50 text-[#9d0b0f] border border-red-200 rounded-xl text-xs font-bold flex items-center gap-1.5">
                              {catName}
                              <button
                                type="button"
                                onClick={() => {
                  ...
                  <div>
                    <label className="block text-[10px] font-black uppercase text-[#88694f] mb-1.5 ml-1">Slogan / Mô tả ngắn</label>
                    <input
                      type="text"
                      className="w-full bg-[#f7f4ef] border-2 border-stone-200 p-3.5 rounded-2xl outline-none focus:border-[#9d0b0f] transition-all font-bold text-[#3e2714]"
              ...
              <div className="p-6 bg-white border border-gray-100 shadow-sm rounded-2xl">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button type="button" onClick={handleAddVariant} className="text-[#f39200] flex items-center gap-1 text-xs font-black hover:underline">
                    <PlusCircle size={16} /> Thêm mức giá
                  </button>
                </div>
                <div className="space-y-3">
                  {formData.variants.map((v, index) => (
                    <div key={index} className="flex gap-3 items-end bg-[#f7f4ef]/50 p-3 rounded-xl border border-dashed border-[#9d0b0f]/20">
                      <div className="flex-1">
                        <label className="text-[10px] font-bold text-gray-400 uppercase">Tên loại / Khối lượng</label>
                        <input
                          required
                      ...
                      <div className="flex-1">
                        ...
                        <input required className="w-full p-2 text-sm border rounded-lg" type="number" value={v.price} onChange={(e) => handleVariantChange(index, 'price', e.target.value)} />
                      </div>
                      <div className="w-24">
                        <label className="text-[10px] font-bold text-gray-400 uppercase">Tồn kho</label>
                        <input required className="w-full p-2 text-sm border rounded-lg" type="number" value={v.stock} onChange={(e) => handleVariantChange(index, 'stock', e.target.value)} />
                      </div>
                      {formData.variants.length > 1 && (
                        <button type="button" onClick={() => handleRemoveVariant(index)} className="p-2 mb-1 text-red-400 hover:text-red-600">
                          <Trash2 size={18} />
                        </button>
                      )}
                    </div>
              ...
              <div className="p-6 bg-white border border-gray-100 shadow-sm rounded-2xl">
                ...
                <div className="grid grid-cols-4 gap-4 md:grid-cols-6">
                  {formData.images.map((img, index) => (
                    <div key={index} className="relative overflow-hidden border aspect-square rounded-xl group">
                      <img src={img} className="object-cover w-full h-full" alt="" />
                      <button
                        type="button"
                        onClick={() => removeImage(index)}
                        className="absolute p-1 text-white transition-all bg-red-500 rounded-full opacity-0 top-1 right-1 group-hover:opacity-100"
                      >
                        <X size={12} />
                      </button>
                    </div>
                  ))}
                  <label className="flex flex-col items-center justify-center transition-all border-2 border-gray-300 border-dashed cursor-pointer aspect-square rounded-xl hover:bg-gray-50">
                    <Plus size={24} className="text-gray-400" />
                    <span className="text-[10px] font-bold text-gray-400 uppercase mt-1">Tải ảnh</span>
              ...
              <button
                ...
                className={`w-full py-4 rounded-xl font-bold uppercase shadow-lg transition-all flex items-center justify-center gap-2 ${
                  isSubmitting ? 'bg-gray-400 text-white cursor-not-allowed' : 'bg-[#9d0b0f] text-white hover:bg-[#f39200]'
                }`}
              >
                {isSubmitting ? (
                  <>
