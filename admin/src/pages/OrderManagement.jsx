...
const OrderManagement = () => {
  ...
  const handleWorkflow = async (orderId, action) => {
    ...
    if (
      !window.confirm(
        actionMessages[action] || "Xác nhận thực hiện hành động này?",
      )
    )
      return;
  ...
  const subtotalForSelected = selectedOrder
    ? (selectedOrder.items || []).reduce((s, it) => s + (Number(it.price || 0) || 0) * (Number(it.quantity || 1) || 0), 0)
    : 0;
    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between border-b-2 border-[#9d0b0f] pb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={fetchOrders}
            className="p-3 bg-white border border-[#9d0b0f]/20 rounded-2xl text-[#9d0b0f] hover:bg-[#f7f4ef] transition-all"
          >
      ...
      <div className="bg-white/80 p-4 rounded-2xl border border-[#9d0b0f]/10">
        <div className="relative">
          ...
          <button 
            onClick={fetchOrders}
            className="absolute right-2 top-1.5 px-4 py-1.5 bg-[#9d0b0f] text-white text-[10px] font-black rounded-xl hover:bg-black transition-all"
          >
            TÌM KIẾM
          </button>
        </div>
      ...
      {showFilters && (
        <div className="p-8 border-2 border-[#9d0b0f]/10 bg-white rounded-[40px] shadow-sm animate-in fade-in slide-in-from-top-4 duration-300">
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={handleFilterChange}
                  className="w-full py-3 pl-12 pr-4 bg-[#f7f4ef] border-0 rounded-[20px] text-sm font-bold"
                />
              </div>
            </div>
          ...
          <div className="flex justify-end gap-3 mt-8 pt-6 border-t border-dashed border-gray-100">
            <button
              onClick={handleResetFilters}
              className="px-8 py-3 text-[10px] font-black uppercase text-gray-500 hover:text-[#9d0b0f] transition-all"
            >
              Thiết lập lại
            </button>
            <button
              onClick={fetchOrders}
              className="px-10 py-3 text-[10px] font-black uppercase text-white bg-[#3e2714] hover:bg-[#9d0b0f] rounded-[20px] shadow-lg shadow-stone-200 transition-all"
            >
              Áp dụng lọc
            </button>
          </div>
      ...
      <div className="bg-white rounded-[32px] shadow-xl border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-4 text-gray-400">
            ...
            <p className="font-bold italic">Không tìm thấy đơn hàng nào phù hợp với bộ lọc.</p>
            <button onClick={handleResetFilters} className="text-[#9d0b0f] underline text-sm font-bold">Xóa bộ lọc và thử lại</button>
          </div>
        ) : (
            ...
            <tbody className="divide-y divide-gray-100">
              {paginatedOrders.map((order) => (
                <tr key={order._id} className="hover:bg-[#f7f4ef]/50 transition-colors">
                        ...
                        <>
                          {order.status === "PENDING" && (
                            <button
                              ...
                              className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-1.5 rounded-xl text-[10px] font-bold shadow-sm"
                            >
                              XÁC NHẬN
                            </button>
                          )}
                          {order.status === "CONFIRMED" && (
                            <button
                              ...
                              className="bg-orange-500 hover:bg-orange-600 text-white px-4 py-1.5 rounded-xl text-[10px] font-bold shadow-sm"
                            >
                              ĐÓNG GÓI
                            </button>
                          )}
                          {order.status === "PACKING" && (
                            <button
                              ...
                              className="bg-green-600 hover:bg-green-700 text-white px-4 py-1.5 rounded-xl text-[10px] font-bold shadow-sm"
                            >
                              BÀN GIAO GHN
                            </button>
                          )}
                  ...
                  <td className="px-8 py-6 text-center">
                    <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase ${getStatusBadge(order.status)}`}>
              ...
              <div className="space-y-6 lg:col-span-1">
                ...
                <div className="bg-white p-6 rounded-[32px] shadow-sm border border-stone-200">
                  ...
                  <div className="space-y-3 text-xs">
                    <p className="font-bold">Phương thức: <span className="text-[#f39200]">{selectedOrder.paymentMethod}</span></p>
                    <p className="flex items-center gap-1 font-bold"><Calendar size={12} /> Ngày đặt: {new Date(selectedOrder.createdAt).toLocaleString("vi-VN")}</p>
              ...
              <div className="lg:col-span-2">
                ...
                <div className="mt-4">
                  ...
                  <div className="flex gap-3 justify-end">
                    {!selectedOrder.ghnOrderCode && !selectedOrder.shipping?.ghnOrderCode && selectedOrder.status !== "CANCELLED" && (
                      <button onClick={async () => {
                        ...
                        try {
                          ...
                          if (d.success) { alert("Đã tạo mã GHN: " + (d.ghnOrderCode || "")); fetchOrders(); setIsModalOpen(false); }
                          else { alert("Lỗi: " + (d.message || "Không thể tạo đơn GHN")); }
                    ...
                    {selectedOrder.status !== "CANCELLED" && selectedOrder.status !== "COMPLETED" && (
                      <button onClick={async () => {
                        ...
                        try {
                          ...
                          const d = await res.json();
                          if (d.success) { alert(d.message || "Đã hủy đơn thành công"); fetchOrders(); setIsModalOpen(false); }
                          else { alert("Lỗi: " + (d.message || "Không thể hủy đơn")); }
                        } catch (e) { alert("Lỗi kết nối"); }
                    ...
                    {selectedOrder.status === "READY_TO_RETURN" && (
                      <button onClick={async () => {
                        ...
                        try {
                          ...
                          if (data.success) { alert("Thành công! Tiền đã được cộng vào ví khách."); fetchOrders(); setIsModalOpen(false); }
                    ...
                    <button onClick={() => { setIsModalOpen(false); navigate(`/orders/track/${selectedOrder._id}`); }} className="px-6 py-2.5 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-wider hover:bg-indigo-700 transition-all flex items-center gap-2">
                      <Eye size={14} /> Xem hành trình
                    </button>
                  </div>
                </div>
                <div className="flex justify-end mt-8 border-t pt-4">
                  <button onClick={() => setIsModalOpen(false)} className="px-8 py-3 rounded-2xl bg-stone-100 text-stone-500 font-bold text-xs hover:bg-stone-200 transition-all">ĐÓNG CỬA SỔ</button>
                </div>
              </div>
            </div>
          </div>
