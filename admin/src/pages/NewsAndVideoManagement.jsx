...
import { Newspaper, Play, Plus, Search, Trash2, Edit3, X, Calendar, MonitorPlay } from 'lucide-react'
...
const NewsAndVideoManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const url = editingItem ? `${API_URL}/api/articles/${editingItem._id}` : `${API_URL}/api/articles`
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-10 text-[#3e2714]">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b-2 border-[#9d0b0f] pb-4 gap-4">
        <div>
          <h2 className="text-3xl font-black text-[#9d0b0f] uppercase tracking-tighter">Quản lý Nội dung</h2>
          <p className="text-[#88694f] font-medium italic text-sm">Cập nhật tin tức tạp chí và thư viện video</p>
        </div>
        <button onClick={() => handleOpenModal()} className="bg-[#9d0b0f] text-white px-6 py-3 rounded-2xl font-bold flex items-center gap-2 shadow-lg hover:bg-black transition-all">
          <Plus size={20} /> {activeTab === 'news' ? 'Viết bài mới' : 'Thêm Video'}
        </button>
      </div>
      ...
      {activeTab === 'news' ? (
        <div className="space-y-4">
          {loading ? (
            ...
            paginatedList.map((item) => (
              <div key={item._id} className="bg-white p-4 rounded-[32px] border border-stone-100 shadow-sm flex flex-col md:flex-row gap-6 hover:shadow-md transition-all group">
                <div className="flex items-center justify-center w-full h-32 overflow-hidden text-gray-400 border md:w-48 rounded-2xl shrink-0 border-stone-100 bg-gray-50">
                  {item.image ? <img src={item.image} className="object-cover w-full h-full transition-transform duration-500 group-hover:scale-110" alt={item.title} /> : <span>No Image</span>}
                </div>
                <div className="flex-1 space-y-2">
                  <div className="flex items-start justify-between">
                    <h3 className="font-black text-lg text-[#3e2714] line-clamp-1">{item.title}</h3>
                    <div className="flex gap-2">
                      <button onClick={() => handleOpenModal(item)} className="p-2 text-blue-600 transition-all bg-blue-50 rounded-xl hover:bg-blue-600 hover:text-white">
                        <Edit3 size={16} />
                      </button>
                      <button onClick={() => handleDelete(item._id)} className="p-2 text-red-600 transition-all bg-red-50 rounded-xl hover:bg-red-600 hover:text-white">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </div>
                  <p className="text-xs italic text-gray-500 line-clamp-2">{item.summary || 'Chưa có tóm tắt...'}</p>
                  <div className="flex items-center gap-4 text-[10px] font-black text-gray-400 uppercase tracking-widest pt-2">
                    ...
                    <span className="text-green-600 bg-green-50 px-2 py-0.5 rounded-full">{item.status}</span>
                  </div>
                </div>
              </div>
            ))
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {paginatedList.map((video) => (
            <div key={video._id} className="bg-white rounded-[32px] border border-stone-100 overflow-hidden shadow-sm hover:shadow-xl transition-all group">
              <div className="relative flex items-center justify-center h-48 overflow-hidden bg-gray-100">
                {video.image ? <img src={video.image} className="object-cover w-full h-full" alt={video.title} /> : <span className="text-gray-400">No Image</span>}
              ...
              <div className="p-6">
                <h3 className="font-black text-[#3e2714] line-clamp-2 mb-4 h-10 text-sm uppercase">{video.title}</h3>
                <div className="flex items-center justify-between pt-4 border-t">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => handleOpenModal(video)} className="text-blue-600 transition-all hover:scale-110">
                      <Edit3 size={16} />
                    </button>
                    <button onClick={() => handleDelete(video._id)} className="text-red-600 transition-all hover:scale-110">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </div>
      ...
      {/* Pagination */}
      {totalPages > 1 && (
        <div className="flex flex-col items-center gap-4 pt-8 mt-12 border-t border-gray-200 border-dashed">
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-[#f7f4ef] w-full max-w-2xl rounded-[40px] shadow-2xl border-2 border-[#9d0b0f] animate-zoomIn overflow-hidden">
            <div className="bg-[#9d0b0f] p-6 text-white flex justify-between items-center">
              <h3 className="flex items-center gap-2 text-xl font-black tracking-tight uppercase">
                {activeTab === 'news' ? <Newspaper /> : <MonitorPlay />}
                {editingItem ? 'Cập nhật' : 'Thêm mới'} {activeTab === 'news' ? 'bài viết' : 'video'}
              </h3>
              <X className="transition-transform cursor-pointer hover:rotate-90" onClick={() => setIsModalOpen(false)} />
            </div>

            <form onSubmit={handleSubmit} className="p-8 space-y-5 max-h-[75vh] overflow-y-auto custom-scrollbar">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-[#88694f] uppercase tracking-widest ml-1">Tiêu đề {activeTab === 'news' ? 'bài viết' : 'video'}</label>
                <input
                  required
              ...
              {activeTab === 'news' ? (
                <>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-[#88694f] uppercase tracking-widest ml-1">Tóm tắt nội dung</label>
                    <textarea
                      rows="2"
                      required
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-[#88694f] uppercase tracking-widest ml-1">Nội dung chi tiết</label>
                    <ReactQuill
                      theme="snow"
                      ...
                      modules={{
                        toolbar: [[{ header: [1, 2, 3, false] }], ['bold', 'italic', 'underline', 'strike'], [{ list: 'ordered' }, { list: 'bullet' }], ['link', 'image'], ['clean']],
                      }}
                    />
                  </div>
                </>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-[#88694f] uppercase tracking-widest ml-1">Link Video (YouTube)</label>
                  <input
                    required
                    className="w-full p-4 rounded-2xl border border-stone-200 outline-none focus:border-[#f39200] text-sm bg-white"
              ...
              <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-[#88694f] uppercase tracking-widest ml-1">Link {activeTab === 'news' ? 'ảnh bìa' : 'thumbnail'}</label>
                  <input
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-[#88694f] uppercase tracking-widest ml-1">Ngày hiển thị</label>
                  <input
                    type="date"
                    className="w-full p-4 rounded-2xl border border-stone-200 outline-none focus:border-[#f39200] font-bold text-sm bg-white"
                    value={formData.date}
              ...
              <button
                type="submit"
                className="w-full bg-[#9d0b0f] text-white py-4 rounded-full font-black uppercase text-xs tracking-[0.2em] hover:bg-[#f39200] transition-all shadow-xl shadow-red-100 mt-4 cursor-pointer"
              >
                Lưu vào hệ thống
              </button>
            </form>
