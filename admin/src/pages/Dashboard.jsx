...
const Dashboard = () => {
  ...
  const filteredByDateOrders = useMemo(() => {
    return orders.filter((order) => {
      ...
      const matchesGlobal = order.customerInfo?.fullName?.toLowerCase().includes(globalSearch.toLowerCase()) || order._id.includes(globalSearch)
  ...
  const barChartData = useMemo(() => {
    const completed = chartOrders.filter((o) => ['COMPLETED', 'DELIVERED'].includes(o.status?.toUpperCase()))
    ...
    if (timeRange === 'DAY') {
      ...
      for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
        ...
        const order = completed.filter((o) => new Date(o.createdAt).toISOString().split('T')[0] === dateKey).reduce((sum, o) => sum + o.totalPrice, 0)
    ...
    const stats = filteredByDateOrders.reduce((acc, o) => {
      if (['COMPLETED', 'DELIVERED'].includes(o.status?.toUpperCase())) {
        o.items.forEach((item) => {
          if (!acc[item.name]) acc[item.name] = { name: item.name, img: item.image, qty: 0 }
  ...
  const totalRevenue = filteredByDateOrders.filter((o) => ['COMPLETED', 'DELIVERED'].includes(o.status?.toUpperCase())).reduce((acc, curr) => acc + curr.totalPrice, 0)
  const finalFilteredOrders = useMemo(() => {
    return filteredByDateOrders.filter((o) => {
      const searchTerm = orderSearch.toLowerCase()
      return o._id.toLowerCase().includes(searchTerm) || o.customerInfo?.fullName?.toLowerCase().includes(searchTerm) || o.customerInfo?.phone?.includes(searchTerm)
    })
  }, [filteredByDateOrders, orderSearch])

  const currentOrders = finalFilteredOrders.slice((orderPage - 1) * itemsPerPage, orderPage * itemsPerPage)

  const filteredUsers = useMemo(() => {
    return users.filter((u) => {
      const searchTerm = userSearch.toLowerCase()
      return u.fullName?.toLowerCase().includes(searchTerm) || u.email?.toLowerCase().includes(searchTerm) || u.role?.toLowerCase().includes(searchTerm)
    })
  }, [users, userSearch])

  const currentUsers = filteredUsers.slice((userPage - 1) * itemsPerPage, userPage * itemsPerPage)
  const filteredVouchers = useMemo(() => {
    return promotions.filter((p) => p.code.toLowerCase().includes(voucherSearch.toLowerCase()))
  }, [promotions, voucherSearch])

  const currentVouchers = filteredVouchers.slice((voucherPage - 1) * vouchersPerPage, voucherPage * vouchersPerPage)

  return (
    <div className="space-y-6 animate-fadeIn pb-10 text-[#3e2714]">
      {/* 1. TOP GLOBAL BAR */}
      <div className="sticky top-0 z-98 flex flex-col xl:flex-row items-center gap-4 bg-white/90 backdrop-blur-md p-4 rounded-[28px] shadow-xl border border-[#9d0b0f]/5">
        <div className="flex items-center gap-3 shrink-0">
          <div className="bg-[#9d0b0f] p-2.5 rounded-xl text-white shadow-lg shadow-red-100">
            <LayoutGrid size={22} />
          </div>
          <h2 className="text-xl font-black text-[#9d0b0f] uppercase tracking-tighter">ClickGo</h2>
        </div>

        <div className="relative flex-1 w-full group">
          <SearchIcon className="absolute left-4 top-1/2 -translate-y-1/2 text-[#9d0b0f]" size={18} />
          <input
            type="text"
        ...
        <button onClick={fetchData} className="p-2.5 bg-white border border-[#9d0b0f]/10 rounded-xl text-[#9d0b0f] hover:bg-red-50">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          text="text-[#f39200]"
        />
        <StatCard to="/orders" label="Đơn hàng kỳ này" val={revenueData.orderCount} icon={<ShoppingCart />} color="bg-red-50" text="text-[#9d0b0f]" />
        <StatCard to="/users" label="Thành viên" val={users.length} icon={<Users />} color="bg-blue-50" text="text-blue-600" />
        <StatCard to="/products" label="Sản phẩm" val={products.length} icon={<Package />} color="bg-emerald-50" text="text-emerald-600" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
          ...
          <div className="flex-1">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={barChartData}>
                ...
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 10, fontWeight: '800', fill: '#88694f' }} />
                <YAxis hide />
                <Tooltip
                  cursor={{ fill: '#f7f4ef' }}
                  contentStyle={{
                  ...
                  formatter={(value, name) => [`${value.toLocaleString()}đ`, name === 'order' ? 'Tiền hàng' : 'Tiền QC']}
                />
        ...
        <div className="lg:col-span-4 bg-white p-4 rounded-[40px] border flex flex-col">
          ...
          <div className="flex-1 px-2 space-y-2 overflow-auto custom-scrollbar">
            {bestSellers.map((p, i) => (
              <div key={i} className="flex items-center gap-3 p-3 bg-[#f7f4ef]/50 rounded-2xl border border-stone-100 hover:bg-[#f7f4ef] transition-all">
                <div className="flex items-center justify-center w-10 h-10 p-1 bg-white shadow-sm rounded-xl">
                  <img src={p.img} className="object-contain max-w-full max-h-full" alt="" />
                </div>
      ...
      <div className="grid grid-cols-1 gap-8 xl:grid-cols-2">
        <TableContainer title="Đơn hàng" search={orderSearch} setSearch={setOrderSearch} page={orderPage} setPage={setOrderPage} total={Math.ceil(filteredByDateOrders.length / 5)}>
          <table className="w-full text-xs text-left">
            ...
            <tbody className="divide-y divide-gray-50">
              {currentOrders.map((o) => (
                <tr key={o._id} className="transition-colors hover:bg-gray-50">
                  <td className="py-4 pl-2 font-bold text-[#9d0b0f]">#{o._id.substring(o._id.length - 4).toUpperCase()}</td>
                  <td className="py-4 font-bold truncate max-w-[120px]">{o.customerInfo?.fullName}</td>
                  <td className="py-4 font-black text-right">{o.totalPrice.toLocaleString()}đ</td>
                  <td className="py-4 text-center">
                    <span
                      className={`text-[8px] font-black uppercase px-2 py-1 rounded-md ${
                        o.status === 'COMPLETED' ? 'bg-green-50 text-green-600' : o.status === 'CANCELLED' ? 'bg-red-50 text-red-600' : 'bg-orange-50 text-[#f39200]'
                      }`}
                    >
        ...
        <TableContainer title="Thành viên" search={userSearch} setSearch={setUserSearch} page={userPage} setPage={setUserPage} total={Math.ceil(filteredUsers.length / 5)}>
          <table className="w-full text-xs text-left">
            <thead className="text-gray-400 uppercase font-black text-[9px] border-b">
              <tr>
              ...
              {currentUsers.map((u) => (
                <tr key={u._id} className="transition-colors hover:bg-gray-50">
                  <td className="flex items-center gap-2 py-3 pl-2">
                    ...
                    <span className="font-bold truncate max-w-[120px]">{u.fullName}</span>
                  </td>
                  <td className="py-3 text-gray-500 italic truncate max-w-[120px]">{u.email}</td>
                  <td className="py-3 text-center">
                    <span className="text-[8px] font-black px-2 py-1 rounded-md uppercase bg-green-50 text-green-600">{u.role}</span>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="bg-white p-8 rounded-[40px] shadow-sm border border-stone-100 flex flex-col min-h-[450px]">
        <div className="flex flex-col items-start justify-between gap-4 mb-8 md:flex-row md:items-center">
          <div>
            ...
            <p className="text-[10px] font-bold text-[#88694f] italic mt-1">Tìm thấy {filteredVouchers.length} mã giảm giá</p>
          </div>
          <div className="flex items-center w-full gap-4 md:w-auto">
            ...
            <Link to="/promotions" className="hidden md:block p-2.5 bg-white border border-stone-200 rounded-xl text-[#9d0b0f] hover:bg-red-50">
        ...
        <div className="grid flex-1 grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
          {currentVouchers.map((promo) => {
            const isOut = promo.usageLimit > 0 && promo.usedCount >= promo.usageLimit
            return (
              <div key={promo._id} className="bg-white rounded-[32px] shadow-sm border border-stone-100 overflow-hidden hover:shadow-xl transition-all flex flex-col group animate-fadeIn">
                <div className={`p-5 text-white bg-gradient-to-br from-[#9d0b0f] to-[#f39200]`}>
                  <h4 className="text-2xl font-black tracking-tighter">{promo.discountType === 'AMOUNT' ? `${promo.discountValue / 1000}K` : `${promo.discountValue}%`}</h4>
                  <p className="font-bold text-[12px] uppercase mt-1 tracking-widest truncate">{promo.code}</p>
                </div>
                <div className="flex flex-col justify-between flex-1 p-5">
                  <div className="space-y-1.5">
                    <div className="flex justify-between text-[9px] font-black uppercase text-gray-400">
                      <span>Đã dùng</span>
                      <span className={isOut ? 'text-red-500' : 'text-[#9d0b0f]'}>
                  ...
                  <p className="text-[10px] text-[#88694f] font-bold italic line-clamp-2 mt-3">{promo.description || 'Ưu đãi ClickGo'}</p>
                </div>
              </div>
            )
          })}
        ...
        {Math.ceil(filteredVouchers.length / vouchersPerPage) > 1 && (
          <div className="flex items-center justify-center gap-6 pt-6 mt-8 border-t border-stone-50">
            <button
            ...
            <span className="text-[11px] font-black text-[#88694f] uppercase tracking-widest">Trang {voucherPage}</span>
            <button
              disabled={voucherPage >= Math.ceil(filteredVouchers.length / vouchersPerPage)}
              onClick={() => setVoucherPage(voucherPage + 1)}
              className="p-2.5 rounded-xl border border-stone-200 text-[#9d0b0f] disabled:opacity-20 hover:bg-red-50 transition-all"
            >
...
const StatCard = ({ label, val, icon, color, text, to }) => (
  <Link to={to} className="bg-white p-6 rounded-[32px] border border-[#9d0b0f]/5 hover:border-[#f39200] transition-all shadow-sm group block">
    <div className="flex items-start justify-between">
      <div className={`w-12 h-12 ${color} ${text} rounded-2xl flex items-center justify-center mb-4 group-hover:rotate-12 transition-transform`}>{icon}</div>
      <ArrowUpRight size={20} className="text-gray-300 group-hover:text-[#f39200]" />
    </div>
...
const TableContainer = ({ title, children, search, setSearch, page, setPage, total }) => (
  <div className="bg-white p-8 rounded-[40px] shadow-sm border border-stone-100 flex flex-col h-[520px]">
    <div className="flex items-center justify-between mb-6">
      ...
      <div className="relative w-40">
        <SearchIcon className="absolute left-3 top-2.5 text-gray-400" size={14} />
        <input
          type="text"
          value={search}
    ...
    <div className="flex items-center justify-between pt-4 mt-4 border-t">
      ...
      <div className="flex gap-1">
        <button disabled={page === 1} onClick={() => setPage(page - 1)} className="p-1 transition-all border rounded-lg hover:bg-gray-50 disabled:opacity-20">
          <ChevronLeft size={14} />
        </button>
        <button disabled={page >= total} onClick={() => setPage(page + 1)} className="p-1 transition-all border rounded-lg hover:bg-gray-50 disabled:opacity-20">
