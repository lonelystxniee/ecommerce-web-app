...
exports.createTopupUrl = async (req, res) => {
  try {
    ...
    if (!tmnCode || !secretKey || !vnpUrl || !returnUrl) {
      console.error("❌ THIẾU CẤU HÌNH VNPAY CHO VÍ TRONG .ENV");
      return res.status(500).json({
        success: false,
        message: "Hệ thống chưa cấu hình đầy đủ tham số VNPay cho ví",
      });
    }
    ...
    if (secureHash === signed) {
      ...
      if (responseCode === "00") {
        ...
        if (transaction && transaction.status === "PENDING") {
          ...
          return res.redirect(
            `${process.env.FRONTEND_URL}/account?topup=success&amount=${transaction.amount}`,
          );
        }
      }
    }
