...
const Checkout = () => {
  ...
  const handleApplyPromo = async () => {
    ...
    setIsApplying(true);
    try {
      const res = await fetch("http://localhost:5175/api/promotions/check", {
        method: "POST",
      ...
      alert("Lỗi kết nối máy chủ khuyến mãi");
    } finally {
      setIsApplying(false);
    }
  };
  ...
  const handleOrder = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5175/api/orders/create", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (data.success) {
        if (formData.paymentMethod === "VNPAY") {
          ...
          const resVnpay = await fetch(
            "http://localhost:5175/api/orders/vnpay-payment",
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
