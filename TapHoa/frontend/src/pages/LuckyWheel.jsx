...
// THÊM ĐẦY ĐỦ CÁC ICON VÀO ĐÂY ĐỂ TRÁNH LỖI "NOT DEFINED"
import {
  ChevronLeft,
  Trophy,
  RefreshCw,
...
const LuckyWheel = () => {
  // --- STATE ---
  const [dbPromos, setDbPromos] = useState([]); // Lưu mã từ backend
  const [myVouchers, setMyVouchers] = useState([]); // Lưu mã người dùng vừa quay trúng
  ...
  useEffect(() => {
    const fetchPromos = async () => {
      try {
        ...
        if (data.success) {
          // Chỉ lấy các mã đang hoạt động (ACTIVE)
  ...
  const wheelPrizes = useMemo(() => {
    ...
    // Gộp mã thật và các ô "May mắn lần sau" để đủ 8 ô
    for (let i = 0; i < 8; i++) {
  ...
  const spinWheel = () => {
    ...
    // Xoay ít nhất 5 vòng + góc ngẫu nhiên
    ...
    setTimeout(() => {
      ...
      // Tính index trúng (kim chỉ ở đỉnh 0 độ)
      const prizeIndex = Math.floor((360 - actualDegree) / segmentAngle) % 8;
      const winner = wheelPrizes[prizeIndex];

      setResult(winner);
      if (winner.code) {
        // CẬP NHẬT: Tự động thêm vào danh sách Voucher của tôi khi trúng
        setMyVouchers((prev) => {
        ...
        alert(winner.name + "! Chúc bạn may mắn lần sau.");
      }
    }, 3500);
  };
    ...
    >
      <div className="max-w-[1200px] mx-auto px-4 pt-10">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-center gap-16">
          ...
          <div className="relative w-[320px] h-[320px] md:w-[480px] md:h-[480px]">
            ...
            >
              {wheelPrizes.map((prize, i) => (
                ...
                >
                  {/* Container chữ: Bây giờ chỉ cần căn giữa theo chiều ngang là xong */}
                  <div className="absolute top-8 md:top-12 left-0 w-full flex justify-center items-start">
                    <span
                      ...
                      style={{
                        ...
                        transform: "rotate(180deg)", // Chữ hướng từ tâm ra ngoài
                        maxHeight: "35%", // Đảm bảo chữ không đè lên nút Quay ở giữa
                        overflow: "hidden",
                      }}
                    >
                      {prize.name}
          ...
          {/* --- PHẦN DANH SÁCH VOUCHER (ĐÃ FIX HIỂN THỊ CHỮ) --- */}
